    ...
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled
            ? "bg-brand-cream/90 shadow-md backdrop-blur-sm"
            : "bg-transparent"
        }`}
      >
        <div className="container mx-auto px-6 py-4 flex justify-between items-center">
          <a
            href="#home"
            className={`text-2xl font-serif font-bold tracking-wider transition-colors duration-300 ${textColorClass}`}
            aria-label="Tiệm Bánh Vi Trần - Trang Chủ"
          >
            Tiệm Bánh Vi Trần
          </a>
          <nav
          ...
          >
            <div className="w-6 h-6 flex flex-col justify-around">
              <span
                className={`block h-0.5 w-full bg-current transform transition-transform duration-300 ease-in-out ${
      ...
      <div
        ...
        id="mobile-menu"
        ref={menuRef}
        className={`fixed inset-0 z-40 bg-brand-cream transition-transform duration-500 ease-in-out md:hidden ${
        ...
        >
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="text-3xl font-serif text-brand-brown hover:text-brand-accent transition-colors duration-300"
            >
              {link.label}
            </a>
          ))}
